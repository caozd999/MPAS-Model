...
bath = ds1.bottomDepth.values.reshape((116,4))
bot1 = bath[:,1]
ds2 = xr.open_dataset('output2.nc')
bath = ds2.bottomDepth.values.reshape((116,4))
bot2 = bath[:,1]
...
def plot_datasets(rval, times, fileno, plotdata=True):
   ...
   #                   marker = '.', color = 'g', label='ROMS')
        if (fileno=='1'):
           plot_MPASO([dtime], fileno, 'k-', lw=0.5, label='Cd_marsh=1.0')
        else:
           plot_MPASO([dtime], fileno, 'k-', lw=0.5, label='Cd=2.5e-3')
...
def place_time_labels(times):
    ...
    for atime, ay in zip(times, locs):
        plt.text(8.2, ay, atime + ' days', size=8)
